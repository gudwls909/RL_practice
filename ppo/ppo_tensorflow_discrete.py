...
class PPO(object):
    def __init__(self, state_size,  action_size, sess, learning_rate, discount_factor, replay, epsilon):
        self.state_size = state_size
        self.action_size = action_size
        self.sess = sess
    ...
    def build_actor(self, scope, trainable):
        ...
        with tf.variable_scope(scope):
            ...
            self.act_stochastic = tf.random.multinomial(tf.log(output), num_samples=1)
    ...
    def optimizer(self):
        ...
        loss = -actor_loss + critic_loss - 0.0003 * entropy
    ...
    def train_network(self):
        ...
        #print(self.sess.run(self.actor, feed_dict={self.state: states, self.advantage: gaes, self.actions: actions, self.target: target}))
...
class Agent(object):
    def __init__(self, args, sess):
        ...
        self.ppo = PPO(self.state_size, self.action_size, self.sess, self.learning_rate,
                       self.discount_factor, self.replay, self.epsilon)
        ...
        for iteration in range(self.iteration):
            for _ in range(self.actor):
                ...
                for _ in range(self.timesteps):
                    ...
                    state = state[0]
                    memory.append([state, [action], 5 * reward, next_state, terminal])
                    score += reward
                    state = next_state
                    if terminal:
                        break
            ...
            ' loss', (np.mean(losses)))
            if int(np.mean(scores2[-min(10, len(scores2)):])) >= self.timesteps - 100:
                render = True
                if int(np.mean(scores2[-min(10, len(scores2)):])) >= self.timesteps - 20:
